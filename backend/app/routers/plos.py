...
async def list_plos(program_id: int | None = None, db: AsyncSession = Depends(get_db)):
    return await plo_service.list_plos(db, program_id)


@router.post("", response_model=PLORead, status_code=status.HTTP_201_CREATED)
async def create_plo(data: PLOCreate, db: AsyncSession = Depends(get_db)):
    try:
...
async def get_plo(plo_id: int, db: AsyncSession = Depends(get_db)):
    try:
...
async def update_plo(plo_id: int, data: PLOUpdate, db: AsyncSession = Depends(get_db)):
    try:
...
async def delete_plo(plo_id: int, db: AsyncSession = Depends(get_db)):
    try:
